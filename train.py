...
import time
import sys
from att_blstm_architecture import blstms_att
...
def train():
	...
	with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            model = blstms_att(layers=FLAGS.layers, max_length=FLAGS.max_sentence_length, n_classes=y.shape[1],vocab_size=len(text_vocab_processor.vocabulary_), embedding_size=FLAGS.text_embedding_dim, batch_size=FLAGS.batch_size,l2_reg_lambda=FLAGS.l2_reg_lambda, gamma=FLAGS.gamma, mp=FLAGS.mp, mn=FLAGS.mn, use_ranking_loss=FLAGS.use_ranking_loss)
